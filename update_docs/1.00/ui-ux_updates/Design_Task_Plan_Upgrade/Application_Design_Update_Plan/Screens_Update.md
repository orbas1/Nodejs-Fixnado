...
## Persona Analytics Dashboards & CSV Export (2025-10-29)
...
- **Warehouse Freshness Alerts (2025-11-02):** Admin telemetry console surfaces inline banner and high-severity status pill when OpsGenie incidents trigger via `analytics-freshness-*` aliases. Banner copy: “Warehouse freshness breached — OpsGenie incident ${incidentId} opened for ${domain} dataset. Last ingest ${minutes} minutes ago (threshold ${threshold}).” CTA buttons: “View OpsGenie runbook” (links to `docs/telemetry/ui-preference-dashboard.md#opsgenie-escalation`) and “Acknowledge in OpsGenie” (deep-link to alert). Banner uses severity tokens `alert-critical` (background `fx.status.critical.bg`, text `fx.status.critical.text`) and includes metadata pills for backlog count and failure streak. QA selector `data-qa="analytics-alert-freshness"`; aria-live polite announcement summarises status, and closing banner posts `analytics.alert.dismiss` event with alias + actor metadata.
