...
- **2025-10-31 — Persona Dashboard Toggle Gating**: Added Secrets Manager-aware gating UX with toggle summary card, pilot access request form, audit banner, and telemetry IDs (`feature.toggle.refresh`, `feature.toggle.request_access`, `feature.toggle.audit`). `Screens_Update.md`, `Screens_Update_Logic_Flow.md`, `Web_Application_Design_Update/Dashboard Designs.md`, and drawings now include gated states, QA selectors (`data-qa="dashboard-gate"`, `data-qa="toggle-summary"`), and runbook references so React/Flutter implementations and analytics ops share governed rollout workflows.
- **2025-11-05 — Analytics Governance Catalogue Publishing**: Introduced admin “Analytics Governance” dashboard card with catalogue version tag, publish timestamp, approval triad roster, status pill, and escalation CTAs. Documented markdown preview modal, fallback download workflow, hotkeys (`Shift+O`, `Shift+P`), telemetry IDs (`analytics.catalogue.preview`, `analytics.catalogue.publish`, `analytics.catalogue.fallback`), and QA selectors across `Screens_Update.md`, `Screens_Update_Logic_Flow.md`, `Dashboard Designs.md`, and runbook/QA artefacts (`docs/telemetry/ui-preference-dashboard.md`, `docs/design/handoff/ui-qa-scenarios.csv`). Flutter admin parity backlog now records read-only governance summary tile.
...
- **2025-11-02 — Warehouse Freshness & OpsGenie Escalation:** Documented OpsGenie-enabled warehouse freshness banner states across admin telemetry and Flutter operations dashboards, including incident metadata chips, CTA behaviours, localisation strings, and telemetry IDs. Updated runbook with on-call rehearsal steps and added QA selectors/automation hooks for incident acknowledgement and recovery flows.
