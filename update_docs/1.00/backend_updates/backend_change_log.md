...
## 2025-10-22 — Communications Platform & Quiet-Hour Governance
- Launched dedicated communications domain models (`Conversation`, `ConversationParticipant`, `ConversationMessage`, `MessageDelivery`) with migration `20250221000000-create-communications.js` persisting participant roles, AI assist metadata, delivery audits, quiet-hour windows, and Agora credentials for cross-channel chat.
- Added `services/communicationsService.js`, `controllers/communicationsController.js`, and `/api/communications` router orchestrating conversation lifecycle, participant enrolment, message send/receive, AI suggestion retrieval, quiet-hour suppression overrides, delivery acknowledgements, and Agora session token minting with deterministic fallbacks when external providers are unavailable.
- Expanded Vitest suite (`tests/communicationsRoutes.test.js`) covering AI assist heuristics vs provider responses, quiet-hour enforcement/override flows, message delivery reconciliation, and Agora token expiry handling to ensure sqlite/Postgres parity and audit-ready logging.

## 2025-10-24 — Unified Analytics Event Schema & Instrumentation
- Introduced `AnalyticsEvent` Sequelize model with domain/entity metadata, actor context, tenant resolution, correlation IDs, and schematised JSON payload storage; migration `20250223000000-create-analytics-events.js` provisions governed indexes on (`tenantId`, `domain`, `occurredAt`) and JSONB metadata paths for Looker ingestion while maintaining sqlite compatibility.
- Added `services/analyticsEventService.js` exporting an immutable event catalogue, validation helpers, tenant inference, timestamp normalisation, and bulk record utilities so emitters share consistent governance and error handling.
- Updated `services/zoneService.js`, `bookingService.js`, `rentalService.js`, `campaignService.js`, and `communicationsService.js` to emit catalogued events for creation, status transitions, assignment lifecycle, disputes, inspections, fraud signals, and delivery suppressions with correlation IDs aligned to dashboards and warehouse requirements.
- Extended Vitest suites (`tests/zoneRoutes.test.js`, `tests/bookingRoutes.test.js`, `tests/rentalRoutes.test.js`, `tests/campaignRoutes.test.js`, `tests/communicationsRoutes.test.js`) to assert analytics event persistence and payload fidelity alongside functional outcomes, keeping telemetry governance under CI coverage.
