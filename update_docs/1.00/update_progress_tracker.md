...
| Task 3 — Marketplace, Inventory & Monetisation Backbones | 68 | 58 | 56 | 60 | 48 | 52 | 57 | Rental lifecycle delivered end-to-end: `/api/rentals` plus `/api/inventory` orchestration covers reservation locking, approvals, checkout, partial returns, inspections, settlement, cancellation, and alert governance (`inventoryService.js`, `rentalService.js`, controllers, routes). Migration `20250217000000-create-inventory-and-rentals.js` seeds inventory/rental tables with deposit, inspection, and audit metadata; Vitest suites (`tests/rentalRoutes.test.js`, `tests/inventoryRoutes.test.js`) assert edge cases (inspection disputes, partial returns, alert escalation). Design docs (`Screens_Update.md`, `Screens_Update_Logic_Flow.md`, `Dashboard Designs.md`) and trackers updated so provider/admin UX can surface rental agreements, inspection queues, and reconciliation tasks aligned to the new APIs. |
...
- Finalise rental document purge automation design (DPIA action) to incorporate inspection evidence retention limits and record sign-off inside security baseline appendix.
- Update support knowledge base with telemetry opt-out instructions and link from Admin telemetry dashboard help panel.
- Prepare joint legal/marketing DPIA walk-through (22 Oct) ensuring rental agreement copy, deposit disclosures, and inspection notices propagate to React + Flutter teams.
- Partner with frontend squads to implement `/api/inventory` health + `/api/rentals` agreement detail panels using refreshed design specs; capture QA instrumentation for alert acknowledgements, inspection submissions, and settlement outcomes.
