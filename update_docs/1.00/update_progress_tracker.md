...
| Task 4 — Cross-Channel Experience & Collaboration | 86 | 84 | 82 | 83 | 76 | 82 | 83 | Provider and enterprise dashboards now surface live metrics via `/provider/dashboard`, `/enterprise/panel`, and `/api/business-fronts/:slug` with RBAC-aware navigation. React routes, mega-menu, and panel pages hydrate bookings, inventory, fraud, and compliance telemetry with skeleton, error, and token handling aligned to backend tests. Panel client now provides cache-aware fallbacks and abortable fetches so `/providers/:slug` builds succeed even during API downtime, while business fronts expose operational showcases/support channels. Remaining scope concentrates on accessibility/localisation sweeps (Subtask 4.5) and mobile parity instrumentation. |
| Task 5 — Analytics, Data Governance & Reporting | 84 | 66 | 64 | 66 | 61 | 62 | 66 | Analytics ingestion pipeline now batches governed events to warehouse endpoints with retry/backoff, retention enforcement, and purge/backfill tooling (`analyticsIngestionJob`, `analyticsEventService` helpers, augmented migration). Communications suppression telemetry defers until post-commit with delivery IDs so dashboards capture quiet-hour outcomes. Vitest coverage verifies ingestion success/failure paths, purge/backfill cadence, and communications regression; docs/design artefacts updated with ingestion swim lanes, support playbooks, and new env/config knobs. |
