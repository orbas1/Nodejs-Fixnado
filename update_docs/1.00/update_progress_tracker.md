...
| Task 5 — Analytics, Data Governance & Reporting | 98 | 97 | 96 | 97 | 95 | 97 | 97 | Warehouse freshness monitor now drives OpsGenie incidents off real ingestion metrics: background job inspects dataset recency, backlog depth, and pipeline failure streaks while configurable thresholds and OpsGenie responders live in `config/index.js`. Service wrapper handles alert lifecycle, alias dedupe, and disabled-integration fallbacks; Vitest suite exercises stale detection, recovery, backlog breach, and pipeline regression scenarios. Telemetry dashboard/runbook copy now references OpsGenie handoff steps so analytics ops rehearse incident routing alongside export parity. |
...
- Operationalise analytics monitoring automations (5.4) and documentation publishing (5.5) using the newly validated mobile exports as rehearsal fixtures.
- Rehearse OpsGenie warehouse freshness alerts in staging, validate responder runbook steps, and confirm Flutter/React banners close automatically after recovery.
