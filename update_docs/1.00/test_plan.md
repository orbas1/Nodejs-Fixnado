...
| Warehouse Freshness & OpsGenie Escalation | Operational monitoring + incident rehearsal | Data Engineering Ops + SRE On-call + Design Systems | Vitest (`tests/warehouseFreshnessJob.test.js`), OpsGenie sandbox webhook, Playwright banner assertion | Background job flags stale/backlog/failure streak states, raises OpsGenie alert with correct alias/tags, dashboard + Flutter banners surface incident metadata, CTAs deep-link to runbook/OpsGenie, telemetry events (`analytics.alert.runbook`, `.acknowledge`, `.recovered`) emitted, and alert auto-closes once pipeline recovers. |
...
| Persona Analytics Dashboards | Functional + export validation | Frontend QA + Analytics Ops + Finance Ops | Vitest + Supertest + CSV diff harness + Playwright (follow-up) | `/api/analytics/dashboards/:persona` aggregates bookings, rentals, campaigns, fraud, compliance, and communications telemetry per role with correct trends/deltas; React `RoleDashboard` renders KPI cards, pipeline widgets, offline banners, and feature toggle gate (`DashboardAccessGate`) when cohort disabled; CSV export mirrors UI metrics with metadata headers (persona, timezone, window) and row count parity. Validation covers error states, localisation, accessibility announcements, and pilot access workflow telemetry (`feature.toggle.refresh`, `feature.toggle.request_access`). |
...
| Load & Resilience Baseline | Performance validation | QA Lead + SRE + Data Engineering | k6 CLI + `scripts/run-load-tests.mjs` orchestrator | Execute `npm --prefix backend-nodejs run load:test -- --profile performance/profiles/baseline.json` with staging data to validate booking/chat/Agora/escrow/analytics/campaign flows stay within profile thresholds (p95 latency, success rate ≥99%, error budget <1%). Harness exports summary JSON to `performance/reports/` for audit and seeds follow-up soak/chaos drills. |
...
9. **k6 Performance Harness**: Install k6 in controlled environments and invoke `scripts/run-load-tests.mjs` with governed profiles (`performance/profiles/baseline.json`) to baseline concurrency envelopes, capture JSON summaries, and feed regression dashboards; harness validation is tracked under Task 6.3 with follow-up soak + chaos variants queued.
