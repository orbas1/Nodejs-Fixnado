...
function matchesFilter(booking, filterStatus) {
  ...
  if (filterStatus === 'pending') {
    return booking.status === 'pending' || booking.assignments?.some((assignment) => assignment.status === 'pending');
  }
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
            ...
            <label className="text-sm font-medium text-primary">
              ...
              <input
                type="search"
                placeholder="Search by title, customer, or tag"
            ...
            >
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                  {booking.zone?.name ? <span className="rounded-full bg-slate-100 px-2 py-0.5">{booking.zone.name}</span> : null}
                  <span className="rounded-full bg-slate-100 px-2 py-0.5">{booking.assignments?.length ?? 0} crew</span>
                </div>
              </div>
              {booking.summary ? (
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-500">
                ...
                <span>
                  Commission: {formatCurrencySafe(booking.financial?.commissionAmount, booking.financial?.currency)}
                </span>
