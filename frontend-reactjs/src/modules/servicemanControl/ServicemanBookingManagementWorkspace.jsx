...
import ServicemanBookingManagementProvider, {
  useServicemanBookingManagement
} from './ServicemanBookingManagementProvider.jsx';
...
function WorkspaceBody() {
  ...
  const selectedBooking = useMemo(() => {
    ...
    return (
      filteredBookings.find((booking) => booking.bookingId === selectedBookingId) ?? filteredBookings[0]
    );
  }, [filteredBookings, selectedBookingId]);

  const summary = workspace?.summary ?? null;
  const servicemanId = workspace?.servicemanId ?? '—';
      ...
      <header className="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-primary">Serviceman workspace</h1>
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-500">
              <StatusPill tone="info">Serviceman ID · {servicemanId}</StatusPill>
...
function ServicemanBookingManagementWorkspace({ initialWorkspace }) {
  return (
    <ServicemanBookingManagementProvider initialWorkspace={initialWorkspace}>
      <WorkspaceBody />
    </ServicemanBookingManagementProvider>
  );
}

ServicemanBookingManagementWorkspace.propTypes = {
  initialWorkspace: PropTypes.object
};

export default ServicemanBookingManagementWorkspace;
