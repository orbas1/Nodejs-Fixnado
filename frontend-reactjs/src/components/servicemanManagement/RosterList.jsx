...
export default function RosterList({ roster, onCreateProfile, onEditProfile, onPlanShift, onAddCertification }) {
  return (
    <div className="rounded-3xl border border-accent/10 bg-white p-6 shadow-md">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-primary">Crew roster</h3>
          <p className="text-sm text-slate-600">Manage contact details, coverage, and upcoming shifts.</p>
        </div>
        <button
          type="button"
      ...
      <ul className="mt-6 space-y-4">
        {roster.length === 0 ? (
          <li className="rounded-2xl border border-dashed border-accent/30 bg-secondary px-4 py-6 text-sm text-slate-500">
            No servicemen have been registered yet. Add your first crew member to build the roster.
          </li>
        ) : (
          roster.map((member) => (
...
RosterList.propTypes = {
  ...
  onPlanShift: PropTypes.func.isRequired,
